...
class Simulation:
	...
	def run_simulation(self):
		""" runs simulation of dining philosophers """
		dp = dining.DiningTable(self.n)
		# initiate metrics class as well that takes in Dining Philosophers
		# instance?
		...
		for t in range(self.time):
			...
			# loop through the philosophers
			for philosopher, phil_id in zip(dp.philosophers,
											range(len(dp.philosophers))):
				# print("Initial", phil_id, ":", philosopher)
				if (philosopher.time > 0):
					philosopher.time = philosopher.time - 1
				elif (philosopher.time == 0):
					if (philosopher.state == PhilosopherState.THINKING):
						# try to pick up
						if (self.get_hungry(dp.n_chairs) and dp.pick_up(
								phil_id,
								phil_id == 3)):
							philosopher.state = PhilosopherState.WAITING
						...
						# try to pick up another chopstick, if successful,
						# then change state to EATING, otherwise drop
						# chopstick and change to THINKING
						if (dp.pick_up(phil_id, phil_id == 3)):
							philosopher.state = PhilosopherState.EATING
							philosopher.time = self.get_eating_time(
								self.eat_function, phil_id)
						else:
							dp.put_down(phil_id)
							philosopher.state = PhilosopherState.THINKING
